...
use Minetro\Formatter\IFormatter;
use Minetro\Formatter\NumberFormatter;
use Tester\Assert;
...
test(function () {
    ...
    Assert::true($formatter instanceof IFormatter);
});
...
test(function () {
    ...
    $formatter->setCallback(function ($prefix, $value, $suffix) {
        return array($prefix, $value, $suffix);
    });
